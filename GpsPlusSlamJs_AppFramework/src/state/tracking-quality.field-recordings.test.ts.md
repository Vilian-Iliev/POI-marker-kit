...
| F2          | "compassAgreement is null at every captured snapshot" (×2)        | Empirical proof that the field hardware never reports `absolute === true` → §4.3 compass cross-check cannot be validated → confirms the v1 deferral decision.                                       |
