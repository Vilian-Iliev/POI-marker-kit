...
import { changeState, selectMarkerState } from './markers/markerStateMachine.ts';
import { store } from './markers/store.ts';
import { initializeGallery, updateGallery, disposeGallery } from './marker-gallery/galleryInitializer.ts';
...
window.addEventListener('keydown', (event) => {
    if(event.key=='Enter'){
       const markerArray = store.getState().markerState;
        markerArray[index].object3d.visible=false;
        index=(index+1)%store.getState().markerState.length;
        markerArray[index].object3d.visible=true;
