...
- `discoverScenariosFromZipMetadata` attaches `h3Cells` to each `SessionEntry` from the `session.json` `h3Cells` field, parsed defensively (`parseH3Cells`: only an array of strings is accepted; non-string entries are dropped; a missing/malformed field yields `undefined`). An empty array is preserved (means "no GPS coverage"), distinct from `undefined` (legacy — needs backfill). Consumed by `recording-index.ts` to build the map browser's coverage index.
